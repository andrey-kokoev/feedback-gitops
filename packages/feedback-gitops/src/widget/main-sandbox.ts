...
const FeedbackWidgetElement = defineCustomElement(FeedbackWidgetCE, {
  configureApp(app) {
    app.use(pinia)
    // IMPORTANT: Inject mock adapter after pinia so the adapter can use stores
    app.provide('widget-adapter', createMockAdapter())
  },
})
...
function bootstrap() {
  console.log('[Widget Sandbox] Bootstrap started')
  ...
  try {
    ...
    console.log('[Widget Sandbox] Config found:', config)
    ...
    console.log('[Widget Sandbox] Element appended to body')
  } catch (err) {
    console.error('[Widget Sandbox] Boot failure:', err)
