...
async function resolvePullRequestAgentWorkState(env, pullRequestNumber) {
    ...
    for (const event of timeline) {
        ...
        if (!eventName.startsWith("copilot_work_") || !createdAt)
            continue;
        const eventTs = Date.parse(createdAt);
        if (Number.isNaN(eventTs))
            continue;
        if (eventTs <= latestEventAt)
            continue;
...
async function withPullRequestAgentWorkState(env, pullRequest) {
    try {
...
async function enableAndConfirmAutoMergeForIssue(env, issueNumber, pullRequest) {
    try {
        await enableAutoMergeForPullRequest(env, pullRequest.id);
    }
    catch (error) {
        const message = getErrorMessage(error, "Failed to enable auto-merge");
        throw new ActionError("AUTO_MERGE_ENABLE_FAILED", `Failed to enable auto-merge for pull request #${pullRequest.number}: ${message}`);
    }
    let refreshed = null;
    try {
    ...
    if (!refreshed || refreshed.number !== pullRequest.number || refreshed.state !== "OPEN") {
        throw new ActionError("AUTO_MERGE_CONFIRM_FAILED", `Unable to confirm auto-merge state for pull request #${pullRequest.number}.`);
    }
    if (!refreshed.autoMergeRequestedAt) {
        throw new ActionError("AUTO_MERGE_NOT_ENABLED", `GitHub did not report auto-merge as enabled for pull request #${pullRequest.number}. Retry merge request.`);
    }
    return refreshed;
}
...
async function executeAction(env, issueNumber, target, action) {
    ...
    if (action === "merge") {
        ...
        if (pullRequest.isDraft) {
            if (pullRequest.agentWorkState === "working") {
                throw new ActionError("AGENT_WORKING", "Copilot is still working on this draft pull request.");
            }
            await markPullRequestReadyForReview(env, pullRequest.id);
            await markIssueMergeRequested(env, issueNumber);
