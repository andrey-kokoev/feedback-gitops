...
function buildIssuePayload(submission, content, defaultLabels) {
    ...
    if (submission.input.type === "audio") {
        ...
        if (submission.input.durationMs)
            contextLines.push(`Audio-Duration-Ms: ${submission.input.durationMs}`);
        if (submission.input.mimeType)
            contextLines.push(`Audio-Mime-Type: ${submission.input.mimeType}`);
    }
    else {
