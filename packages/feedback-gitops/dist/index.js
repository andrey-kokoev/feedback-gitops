// src/consumer.ts
function createIssueConsumer(config) {
  return async (batch) => {
    const failures = [];
...
async function resolveRequestContent(submission, audio) {
...
function deriveTitleFromTranscript(transcript) {
...
function buildIssuePayload(submission, content, defaultLabels) {
  const policyLabel = submission.mergePolicy === "auto_unblocked" ? "agent-policy-auto-merge" : "agent-policy-manual-merge";
  const labels = [.../* @__PURE__ */ new Set(["agent-change-request", policyLabel, ...defaultLabels || [], ...(submission.labels || []).filter(Boolean)])];
  const contextLines = [];
  ...
  contextLines.push(`Timestamp: ${(/* @__PURE__ */ new Date()).toISOString()}`);
...
// src/inline-recorder.ts
function generateInlineAudioRecorderSource() {
  return `
...
// src/widget.ts
function generateWidgetScript(endpoint, defaultRepo, defaultLabels) {
...
@media (min-width: 681px) {
  ...
  function renderIssues() {
    ...
    state.issues.forEach((issue) => {
      ...
      if (!issue.pullRequest || !issue.pullRequest.url) {
        ...
        prLink.textContent = '#' + issue.pullRequest.number + ' \xB7 ' + String(issue.pullRequest.state || '').toLowerCase();
      ...
      if (issue.pullRequest && issue.pullRequest.url) {
        ...
        const prStateText = String(issue.pullRequest.state || '').toLowerCase() + (issue.pullRequest.isDraft ? ' \xB7 draft' : '');
        ...
        prStateTd.textContent = prStateText
          + (prStatusDetail ? ' \xB7 ' + prStatusDetail : '')
          + (prAgentLabel ? ' \xB7 ' + prAgentLabel : '');
      ...
      cardHead.innerHTML = '<strong>#' + issue.number + '</strong><span>' + statusText + (cardStatusDetail ? ' \xB7 ' + cardStatusDetail : '') + '</span>';
      ...
      if (issue.pullRequest && issue.pullRequest.url) {
        cardMeta.innerHTML = '<a class="cfw-link" href="' + issue.pullRequest.url + '" target="_blank" rel="noopener noreferrer">PR #' + issue.pullRequest.number + ' \xB7 ' + String(issue.pullRequest.state || '').toLowerCase() + '</a>';
      } else {
  ...
  function renderMobileIssues() {
    ...
    state.issues.forEach((issue) => {
      ...
      if (issue.pullRequest && issue.pullRequest.url) {
        meta.textContent = 'PR #' + issue.pullRequest.number + ' \xB7 ' + String(issue.pullRequest.state || '').toLowerCase();
      } else {
  ...
  function openMobileIssueSheet(issue) {
    ...
    statusEl.textContent = (issue.status || issue.state || '') + (issue.statusDetail ? ' \xB7 ' + issue.statusDetail : '');
    ...
    if (issue.pullRequest && issue.pullRequest.url) {
      ...
      prLink.target = '_blank';
      prLink.rel = 'noopener noreferrer';
      const prState = String(issue.pullRequest.state || '').toLowerCase() + (issue.pullRequest.isDraft ? ' \xB7 draft' : '');
      prLink.textContent = 'PR #' + issue.pullRequest.number + ' \xB7 ' + prState;
      sec.appendChild(lbl);
